...
**CM Invoice Tracking** 是一个企业级的发票工作流管理和团队协作平台，旨在帮助大型汽车供应链企业（如 BYD、Tesla、BMW、Mercedes-Benz 等）实现发票处理的全过程跟踪、状态管理和团队协作。
...
- Green（已完成）：Actual Date 不为空
- Red（已逾期）：Actual Date 为空且 Due Date < 今天
- Yellow（待处理）：Actual Date 为空且 Due Date >= 今天
...
- **Workflow Health / Progress**：Normal / Abnormal / Done - 进度健康度
- 三种状态使用彩色圆点直观表示
